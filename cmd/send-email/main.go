...
// This type allows for string list flags, for supporting multiple send to emails.
...
func (e *emailList) Set(val string) error {
	*e = append(*e, strings.Split(val, ",")...)
	return nil
}
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;">
  <tr><td style="background:#1a1a2e;padding:24px 32px;">
    <h1 style="margin:0;color:#fff;font-size:22px;">Claude's Weekly AI News Digest</h1>
  ...
  <tr><td style="padding:16px 32px;background:#f8f8f8;text-align:center;">
    <p style="margin:0;color:#999;font-size:12px;">Generated by Claude - AI Weekly News Digest</p>
  </td></tr>
</table>
</td></tr>
</table>
...
var plainTmpl = template.Must(template.New("plain").Parse(`Claude Weekly AI News Digest — {{.SearchDate}}
========================================
{{range .Items}}
[{{.Category}}] {{.Title}}
{{.Summary}}
...
{{end}}
---
Generated by Claude - Weekly AI News Digest
`))
...
func main() {
	...
	var to emailList
	flag.Var(&to, "to", "Comma-separated list of recipient email addresses")
	flag.Parse()

	if *host == "" || *user == "" || *password == "" || *from == "" || len(to) == 0 {
	...
	for _, toAddr := range to {
		...
		m.Subject(fmt.Sprintf("Claude Weekly AI News Digest — %s", digest.SearchDate))
	...
	if err := c.DialAndSend(messages...); err != nil {
		fmt.Fprintf(os.Stderr, "error sending email: %v\n", err)
		os.Exit(1)
	}

	fmt.Printf("Emails sent to %v (%d items)\n", to, digest.ItemCount)
}
